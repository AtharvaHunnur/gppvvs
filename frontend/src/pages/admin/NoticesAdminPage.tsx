...
const NoticesAdminPage = () => {
    ...
    },
    {
      ...
      render: (row: any) => (
        <div className="flex justify-end gap-1">
          <button
            onClick={() => setSelectedNoticeId(selectedNoticeId === row.id ? null : row.id)}
            className={`p-2 rounded-lg transition-colors ${selectedNoticeId === row.id ? 'text-primary bg-primary-50' : 'text-text-secondary hover:text-primary'}`}
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Document Upload Section for selected notice */}
      {selectedNoticeId && (
        <DocumentUploadSection section="notices" entityId={selectedNoticeId} label="Notice Documents" />
      )}
