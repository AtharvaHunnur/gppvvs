...
const PagesAdminPage = () => {
  ...
  const fetchPages = async () => {
    try { const res = await apiClient.get('/pages'); setPages(res.data.data); }
  ...
  const handleSelectPage = (page: any) => {
    setSelectedPage(page); setTitle(page.title); setContent(page.content);
    if (page.images) {
      try { const parsed = typeof page.images === 'string' ? JSON.parse(page.images) : page.images; setImages(Array.isArray(parsed) ? parsed : []); }
      catch (e) { setImages([]); }
    } else { setImages([]); }
  };

  const handleAddImage = () => { if (!newImageUrl.trim()) return; setImages([...images, newImageUrl.trim()]); setNewImageUrl(''); };
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    setSaving(true);
    try {
      await apiClient.put(`/pages/${selectedPage.id}`, { title, content, images: JSON.stringify(images) });
      showMsg('success', 'Page updated successfully!'); fetchPages();
    } catch (error) { console.error('Failed to update page', error); showMsg('error', 'Failed to update page.'); }
    finally { setSaving(false); }
  };

  if (loading) return <div className="p-8 text-center text-text-secondary">Loading pages editor...</div>;
        ...
        <div className="space-y-6">
          <div className="bg-white rounded-2xl shadow-sm border border-surface-200 overflow-hidden">
            ...
            <form onSubmit={handleSave} className="p-6 space-y-6">
              ...
              <div>
                <label className="block text-sm font-bold text-text mb-2">Page Images Slider</label>
                <div className="space-y-4">
                  <div className="flex gap-2">
                    <input type="text" placeholder="Enter image URL (e.g. /images/about_building.png)" value={newImageUrl} onChange={(e) => setNewImageUrl(e.target.value)} className="flex-1 px-4 py-3 rounded-xl border border-surface-200 focus:outline-none focus:border-primary-400 text-sm" />
                    <button type="button" onClick={handleAddImage} className="px-4 py-3 bg-secondary text-primary-900 font-bold rounded-xl hover:bg-secondary-50 transition flex items-center text-sm">
                      ...
                      {images.map((imgUrl, index) => (
                        <div key={index} className="relative group rounded-xl overflow-hidden shadow-sm aspect-video bg-white border border-surface-200">
                          ...
                          <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                            <button type="button" onClick={() => handleRemoveImage(index)} className="bg-red-600 hover:bg-red-700 text-white p-2 rounded-full shadow-lg transition"><Trash2 size={16} /></button>
                          </div>
                          <div className="absolute bottom-2 left-2 right-2 bg-black/60 px-2 py-1 rounded text-[10px] text-white truncate">{imgUrl}</div>
                        </div>
                      ))}
                    </div>
                  ) : (
              ...
              <div className="flex justify-end pt-4 border-t border-surface-200">
                <button type="submit" disabled={saving} className="px-6 py-3 bg-primary text-white font-bold rounded-xl hover:bg-primary-800 transition flex items-center">
